// src/pages/AuthorAuthPage.jsx

import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import AuthForm from '../components/AuthForm'; // 재사용 가능한 인증 폼 컴포넌트 임포트
import { joinAuthor } from '../api/bookApi'; // 작가 회원가입 API 임포트
...
const AuthorAuthPage = () => {
   ...
   * @param {object} formData - 폼에서 입력된 데이터 (loginId, password, name, portfolioUrl)
   */
  const handleSubmit = async (formData) => {
    ...
    try {
      if (mode === 'signup') {
        // 작가 회원가입 API 호출
        const response = await joinAuthor(formData);
        console.log('Author Signup Success:', response.data);
        alert('작가 회원가입이 완료되었습니다! 관리자 승인 후 로그인해주세요.'); // 간단한 알림
        ...
        console.log('Author Login Attempt:', formData);

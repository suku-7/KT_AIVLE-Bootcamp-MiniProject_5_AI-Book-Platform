    ...
    <v-container>
        ...
        <div class="panel">
            <div class="gs-bundle-of-buttons" style="max-height:10vh;">
                ...
                <v-btn :disabled="!selectedRow" style="margin-left: 5px;" @click="registBook" class="contrast-primary-text" small color="primary" :disabled="!hasRole('author')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>책 등록
                </v-btn>
                <v-btn :disabled="!selectedRow" style="margin-left: 5px;" @click="modifyContext" class="contrast-primary-text" small color="primary" :disabled="!hasRole('author')">
                ...
                <v-table>
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td class="whitespace-nowrap" label="Registration">{{ val.registration }}</td>
                ...
                <v-card>
                    ...
                    <v-card-text>
                        <div>
                            ...
                            <Boolean label="Registration" v-model="selectedRow.registration" :editMode="true"/>
...
export default {
    ...
    methods:{
        async registBook(){
            try{
                var path = "registBook".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, null)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','registBook 성공적으로 처리되었습니다.')
        ...
        async modifyContext(){
            try{
                var path = "modifyContext".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, null)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','modifyContext 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
